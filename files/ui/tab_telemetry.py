...
class TelemetryTabMixin:
    ...
    CDEFS = {
        ...
        "Tire Temps (Front)": (['LFtempCL', 'LFtempCM', 'LFtempCR', 'RFtempCL', 'RFtempCM', 'RFtempCR'],
                               ...
                               ['#3498db', '#2980b9', '#1a5276', '#e74c3c', '#a03010', '#922b21']),
    ...
    def _draw_stb_chart(self):
        ...
        if inc_ch is not None and ld is not None and laps:
            inc_positions = []
            for lap_idx in laps:
                if lap_idx + 1 >= len(d.lap_boundaries):
                    continue
                s_i, e_i = d.lap_boundaries[lap_idx], d.lap_boundaries[lap_idx + 1]
                if e_i > len(inc_ch):
                    continue
                lap_inc = inc_ch[s_i:e_i]
                lap_ld = ld[s_i:e_i]
                for i in range(1, len(lap_inc)):
                    if lap_inc[i] > lap_inc[i - 1]:
                        inc_positions.append(float(lap_ld[i]) * 100.0)
            for pos in inc_positions:
                for ax in axes:
                    ax.axvline(pos, color='#f39c12', lw=1.4, alpha=0.75,
                               ls=':', zorder=6)
            if inc_positions:
                axes[0].text(0.01, 0.97,
                             f"⚠ {len(inc_positions)} incident{'s' if len(inc_positions) != 1 else ''}",
                             transform=axes[0].transAxes, fontsize=8,
                             color='#f39c12', va='top', alpha=0.9)
